...
with open("week 7\simple-2.csv") as csvfile: 
    ...
    for rec in file: 
        names.append(rec[0])
        nums.append(rec[1])
        colors.append(rec[2].title()) #just for Ray in the morning session, capitalizes the first character in a string or the first character after a space
...
while ans == "y":
    ...
    if choice not in valid_menu: 
        ...
        #bubble sort -- always sort BEFORE binary search!!!!!!!!!!
        ...
        while min < max and search.lower() != names[mid].lower():
            if search.lower() < names[mid].lower():
                ...
                # search is greater than names[mid]
                min = mid + 1
